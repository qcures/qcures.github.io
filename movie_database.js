...
for (let i = 0; i < movies.length; i++) {
  ...
  filterButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      filterMovies(filter); // Call the filterMovies function with the selected filter category
      highlightButton(button); // Highlight the active button
    });
  });

  // Function to filter the movies based on the selected category
  function filterMovies(category) {
    ...
    if (category === "all") {
      ...
      filteredMovies = movies.filter((movie) => movie.popularity > 1000); // Filter movies with popularity over 1000 (example)
    } else if (category === "oldies") {
      filteredMovies = movies.filter(
        (movie) => movie.release_date < "2000-01-01"
      ); // Filter movies released before 2000 (example)
    } else if (category === "top-rated") {
      filteredMovies = movies.filter((movie) => movie.vote_average >= 8.0); // Filter movies with a vote average of 8.0 or higher (example)
    } else if (category === "new") {
      filteredMovies = movies.filter(
        (movie) => movie.release_date >= "2023-01-01"
      ); // Filter movies released in 2023 or later (example)
    }
  ...
  // Set the initial state to show movies filtered by "All"
  filterMovies("all");
}
